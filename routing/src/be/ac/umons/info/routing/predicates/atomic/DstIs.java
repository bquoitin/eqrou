...
/**
 * Destination equality: this predicate checks whether the route's destination
 * prefix is equal to a given prefix.
 */
public class DstIs implements IPredicate {
	 ...
	 * "192.168.128.0/17".
	 */
	public DstIs(final String prefix) {
	...
	public PredicateAutomaton automaton(RouteAlphabet routeAlphabet) {
		...
		// forbid longer prefix
		rulesDest.add(new BasicRule<ILabel,IState>(
			new IntegerLabel(0), sink, qPrevious));
		rulesDest.add(new BasicRule<ILabel,IState>(
			new IntegerLabel(1), sink, qPrevious));		
		// final states
